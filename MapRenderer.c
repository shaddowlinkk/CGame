            ...
            if(gameData->map[y][x]>=7 && gameData->map[y][x]<=10){
                ...
                if (gameData->map[y][x]==9 || gameData->map[y][x] ==7){
                    ...
                    if(gameData->map[y][x]==8){
                        SDL_RenderCopyEx(rend,ground.tileTexture,&gameData->Tiles[gameData->map[y][x]].tileRect,&ground.tileRect,90,&p,SDL_FLIP_VERTICAL);
                    }else if(gameData->map[y][x]==10){
                        SDL_RenderCopyEx(rend,ground.tileTexture,&gameData->Tiles[gameData->map[y][x]].tileRect,&ground.tileRect,90,&p,SDL_FLIP_NONE);
                    }
                }
            }else{
                SDL_RenderCopy(rend, ground.tileTexture, &gameData->Tiles[gameData->map[y][x]].tileRect,&ground.tileRect);
